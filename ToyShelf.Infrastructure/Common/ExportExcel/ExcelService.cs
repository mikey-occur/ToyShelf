...
namespace ToyShelf.Infrastructure.Common.ExportExcel
{
	public class ExcelService : IExcelService
	{
		public byte[] ExportSettlements(IEnumerable<MonthlySettlementResponse> settlements)
		{
			...
			var headers = new string[] { "Mã Đối Tác", "Tên Đối Tác", "Ngân Hàng", "Số Tài Khoản", "Tên Chủ TK", "Tháng", "Năm", "Tổng Số hàng bán được", "Tổng Tiền Bán", "Tổng Hoa Hồng", "Khấu Trừ", "Thực Nhận", "Trạng Thái", "Ngày Tạo" };
			...
			foreach (var item in settlements)
			{
                ...
                worksheet.Cell(currentRow, 4).Value = item.BankAccountNumber ?? "N/A";
                worksheet.Cell(currentRow, 5).Value = item.BankAccountName ?? "N/A";
                worksheet.Cell(currentRow, 6).Value = item.Month;
                worksheet.Cell(currentRow, 7).Value = item.Year;
                worksheet.Cell(currentRow, 8).Value = item.TotalItems;
                worksheet.Cell(currentRow, 9).Value = item.TotalSalesAmount;
                worksheet.Cell(currentRow, 10).Value = item.TotalCommissionAmount;
                worksheet.Cell(currentRow, 11).Value = item.DeductionAmount;
                worksheet.Cell(currentRow, 12).Value = item.FinalAmount;
                worksheet.Cell(currentRow, 13).Value = item.Status;
                worksheet.Cell(currentRow, 14).Value = item.CreatedAt.ToString("dd/MM/yyyy HH:mm");
                currentRow++;
			}
